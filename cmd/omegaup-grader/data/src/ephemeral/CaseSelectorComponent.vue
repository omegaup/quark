  ...
  <div class="root d-flex flex-column h-100 bg-dark text-white">
      ...
      <div class="list-group">
        ...
        <template v-bind:title="name" v-for="group in groups" v-else>
          <div class="list-group-item list-group-item-secondary" v-if="group.explicit">
            <div>
              <span class="verdict" v-bind:title="verdictTooltip(groupResult(group.name))" v-bind:class="verdictClass(groupResult(group.name))">{{ verdictLabel(groupResult(group.name)) }}<span class="score">{{ score(groupResult(group.name)) }}</span></span>
              {{ group.name }}
            </div>
          </div>
          <button type="button" v-bind:class="{ 'in-group': group.explicit, active: currentCase == item.name }" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center"  v-on:click="selectCase(item.name)" v-for="item in group.cases">
            <div>
              <span class="verdict" v-bind:title="verdictTooltip(caseResult(item.name))" v-bind:class="verdictClass(item.name)">{{ verdictLabel(caseResult(item.name)) }}<span class="score">{{ score(caseResult(item.name)) }}</span></span>
              {{ item.name }}
            </div>
    ...
    <form v-on:submit.prevent="createCase()">
      <div class="input-group">
        <input type="text" class="form-control case-weight" v-model="newCaseWeight"></input>
        <input type="text" class="form-control" v-model="newCaseName"></input>
        <div class="input-group-append">
          <button class="btn btn-secondary" type="submit" v-bind:disabled="newCaseName.length == 1" v-on:click="createCase()">+</button>
        </div>
      </div>
    </form>
  </div>
...
export default {
  ...
  computed: {
    summary: function() {
      if (this.store.state.dirty || !this.store.state.results || !this.store.state.results.verdict)
        return '…';
